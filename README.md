...
**fxDevKit 是研发增强工具**：装一次，你日常的研发动作就被增强了——提交时自动补前缀、自动跳过 merge、自动记录发生了什么。

它做两件事：

1. **看懂上下文** —— 当前仓库、分支、这次是 amend 还是新提交、是不是在 merge 中，它都知道，并把这些结构化地交给插件
2. **在指定处增强** —— 你说增强哪里就增强哪里，不越界、不替你决策，出问题绝不阻断你的动作

**能力全部由插件提供**，内核只负责调度与兜底。想加什么增强，就装什么插件。

三种方式打开它：

| 入口 | 怎么用 | 例子 |
|---|---|---|
| **自动触发** | 你照常敲命令，它在背后执行 | `git commit` → 自动改写 message |
